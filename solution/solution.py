...
def call_openai(
    ...
    latency = max(time.time() - start, 1e-12)
    ...
    for result in results:
        rows.append(
            [
                ...
                truncate(result.get("gpt4o_answer", "")),
                truncate(result.get("mini_answer", "")),
                f"{float(result.get('gpt4o_time', 0.0)):.3f}s",
                f"{float(result.get('mini_time', 0.0)):.3f}s",
            ]
        )
